...
- **World map** — Natural Earth coastlines embedded at build time, rendered with Braille (U+2800–U+28FF) for sub-pixel resolution. **Adaptive detail**: the renderer switches between the 110m / 50m / 10m datasets based on zoom, so zooming in reveals finer coastline (bays, archipelagos, islands) instead of a coarser blow-up. Auto-fits to the bounding box of currently active contacts plus your own position, with cell-aspect and `cos(lat)` compensation so Sweden doesn't get stretched horizontally. The rendered land layer is cached per viewport so the per-frame cost stays low.
